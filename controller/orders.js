...
const addOrder = (req,res)=>{
    const addOrders = "INSERT INTO `orders` (`o_userid`, `o_name`,`o_email`, `o_price`, `o_items`,`o_status`,`o_address`, `o_date`) VALUES (?,?,?,?,?,?,?,?)"
    ...
    db.query(addOrders, values, async(err, data)=>{
        if(err){
            ...
            await ordersModel
            .create({name:clientName, email:clientEmail, code: clientCode, bill: clientBill })
            .then(() => {
...
module.exports = {addOrder, getOrders, getSingleOrder, getSingleOrderImages, updateStatus, deleteOrder}